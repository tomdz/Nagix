...
module Nagix
  class NQLParser < Parslet::Parser
    ...
    rule(:table_columns)  { columns.as(:columns) >> whitespace? >> from >> identifier.as(:name) >> whitespace? }
    rule(:query)          { (select >> table_columns.as(:table) >> where_clause.maybe).as(:query) }
  ...
  class NQLTransformer < Parslet::Transform
    ...
    rule(:name => simple(:name), :columns => sequence(:columns)) { "GET #{name}\nResponseHeader: fixed16\nColumns: #{columns.join(' ')}\nColumnHeaders: on" }
    rule(:name => simple(:name), :columns => simple(:column)) { "GET #{name}\nResponseHeader: fixed16\nColumns: #{column}\nColumnHeaders: on" }
    rule(:name => simple(:name), :columns => { :all => simple(:all) }) { "GET #{name}\nResponseHeader: fixed16" }

    rule(:query => { :table => subtree(:table), :conditions => subtree(:conditions) }) { "#{table}#{conditions}\n" }
    rule(:query => { :table => subtree(:table) }) { "#{table}\n" }
  end
